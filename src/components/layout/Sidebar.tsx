...
import { ChevronDown, Search, Layers, Filter, Hospital, GraduationCap, Globe2, Map as MapIcon, Sliders, X } from 'lucide-react';
import { useApp } from '@/lib/state';
import { loadCountries, loadStats } from '@/lib/data';
import { METRICS, METRIC_BY_KEY, type AdminLevel, type CountriesIndex, type MetricKey, type StatsTable } from '@/lib/types';
import { distinctAmenities, distinctProvinces, formatMetric, metricValues } from '@/lib/utils';
import { useTheme } from '@/lib/theme';

const METRIC_GROUPS: { key: string; label: string }[] = [
  { key: 'population', label: 'Affected Population' },
  { key: 'children',   label: 'Affected Children' },
...
export default function Sidebar() {
  ...
  const [openSection, setOpenSection] = useState<Record<string, boolean>>({
    metric: true, province: false, threshold: false, amenity: false,
  });
  ...
  const Section = ({ id, icon: Icon, title, badge, children }: any) => (
    ...
    <aside style={{ width: 280, background: bg, borderRight: `1px solid ${border}`, display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100%' }}>
      <div style={{ padding: '10px 14px', borderBottom: `1px solid ${border}`, flexShrink: 0 }}>
        ...
        <p style={{ color: tm, fontSize: 9, marginTop: 2, letterSpacing: '0.06em' }}>
          {country ? `${country.totals.district_count.toLocaleString()} districts · ${country.totals.health_count.toLocaleString()} health · ${country.totals.school_count.toLocaleString()} schools` : 'Loading…'}
        </p>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
        ...
        <div style={{ position: 'relative' }}>
          <select
            ...
            style={{ width: '100%', padding: '7px 24px 7px 10px', borderRadius: 8, background: inputBg, border: `1px solid ${border}`, color: tp, fontSize: 12, appearance: 'none', cursor: 'pointer', outline: 'none' }}
          >
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[{ k: 'health' as const, label: 'Health', icon: Hospital, color: '#ef4444' },
            { k: 'schools' as const, label: 'Schools', icon: GraduationCap, color: '#3b82f6' }].map(p => {
        ...
        {availableLevels.includes(1) && (
          <Section id="province" icon={MapIcon} title="PROVINCE FILTER"
            ...
            {provinces.length === 0 && <p style={{ color: tm, fontSize: 10 }}>No provinces.</p>}
            {provinces.length > 0 && (
              <>
                ...
                <div style={{ maxHeight: 180, overflowY: 'auto' }}>
                  {provinces.map(p => {
                    ...
                    return (
                      <label key={p.id}
                        style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 4px', cursor: 'pointer', borderRadius: 4, background: sel ? 'rgba(200,169,81,0.08)' : 'transparent' }}>
                        <input type="checkbox" checked={sel} onChange={() => app.toggleProvince(p.id)} style={{ accentColor: '#c8a951' }} />
                        <span style={{ color: sel ? '#c8a951' : ts, fontSize: 11 }}>{p.name}</span>
                      </label>
                    );
                  })}
                </div>
        ...
        {/* ── Amenity filter (health) ── */}
        <Section id="amenity" icon={Hospital} title="HEALTH AMENITY"
          ...
          {app.search && stats && (
            <div style={{ maxHeight: 160, overflowY: 'auto', marginTop: 4 }}>
              {Object.entries(stats)
                ...
                .slice(0, 25)
                .map(([id, d]) => (
                  <button key={id} onClick={() => app.selectDistrict(id)}
